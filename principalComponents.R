...
score <- mapply(genotypes, z, FUN = function(x,z){
...
# library(pcaMethods)
# zscore <- prep(score, scale="none", center=TRUE)
...
# *** Drop the Japan Sea fish, as they are outliers ***
groups <- groupnames[groupcodes]
score <- score[ groups != "marine-jap", ]

z <- prcomp(score, center = TRUE)

cat("\nProportion of variance accounted for by each PC\n")
print( round( 100*(z$sdev^2)/sum(z$sdev^2), 2) )
...
pcaRes <- cbind.data.frame(fishnames = fishnames[groups != "marine-jap"], 
			groupnames = groups[groups != "marine-jap"], stringsAsFactors = FALSE)
pcaRes <- cbind.data.frame(pcaRes, z$x)

save(pcaRes, file = pcaResFile) 
# load(file = pcaResFile) # object is pcaRes
# load("BenlimAllMarine.chrXXI.pcaRes.rdd") # object is pcaRes
